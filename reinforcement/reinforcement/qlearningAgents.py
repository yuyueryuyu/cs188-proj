...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        if not actions:
            return None
        qVal, qAction = float('-inf'), None
        for action in actions:
            newVal = self.getQValue(state, action)
            if newVal > qVal:
                qVal, qAction = newVal, action
        return qAction
        util.raiseNotDefined()
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.qValues[str(state) + action] = (1 - self.alpha) * self.getQValue(state, action) + self.alpha * (reward + self.discount * self.computeValueFromQValues(nextState))
