...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[OBS0] = range(manhattanDistance((0, 0), (X_RANGE - 1, Y_RANGE - 1)) + MAX_NOISE + 1)
    variableDomainsDict[OBS1] = range(manhattanDistance((0, 0), (X_RANGE - 1, Y_RANGE - 1)) + MAX_NOISE + 1)
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for el in eliminationOrder:
            factors, joinedFactor = joinFactorsByVariable(factors, el)
            if len(joinedFactor.unconditionedVariables()) > 1:
                factors.append(eliminate(joinedFactor, el))
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for ghostPos in self.allPositions:
            ...
            for key in newPosDist:
                newBelief[key] += newPosDist[key] * self.beliefs[ghostPos]
        newBelief.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        s = len(self.legalPositions)
        mod = num % s
        for pos in self.legalPositions:
            self.particles += [pos for _ in range(0, num // s)]
        self.particles += self.legalPositions[:mod]
        "*** END YOUR CODE HERE ***"
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        num = self.numParticles
        s = len(self.legalPositions) ** self.numGhosts
        mod = num % s
        i = 0
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPos in self.particles:
            list = [self.getObservationProb(observation[i], pacPos, ghostPos[i], jailPos[i]) for i in range(self.numGhosts)]
            i = 1
            for el in list:
                i *= el
            beliefs[ghostPos] += i
