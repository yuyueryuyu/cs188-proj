...
def joinFactors(factors: List[Factor]):
    ...
    for factor in factors:
        ...
        for key in dict:
            variableDomainsDict[key] = dict[key]
    ...
    for dict in newFactor.getAllPossibleAssignmentDicts():
        ...
        for factor in factors:
            for d in factor.getAllPossibleAssignmentDicts():
                if all(item in dict.items() for item in d.items()):
                    prob *= factor.getProbability(d)
                    break
        newFactor.setProbability(dict, prob)
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        conditioned.discard(eliminationVariable)
        ...
        for dict in newFactor.getAllPossibleAssignmentDicts():
            prob = 0
            for d in factor.getAllPossibleAssignmentDicts():
                if all(item in d.items() for item in dict.items()):
                    prob += factor.getProbability(d)
            newFactor.setProbability(dict, prob)
